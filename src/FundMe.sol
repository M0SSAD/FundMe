...
import {console} from "forge-std/console.sol"; // For logging in the console.
...
contract FundMe {
    ...
    AggregatorV3Interface private immutable priceFeed; // To get the price of ETH in USDs.
    ...
    address private immutable i_owner; // The owner of the contract.
    ...
    function fund() public payable {
        ...
        s_funders.push(msg.sender); // To keep track of all the funders. 
        s_addressToAmountFunded[msg.sender] += msg.value; // To keep track of the amount funded by each address.
    ...
    function cheaperWithdraw() public onlyOwner {
        uint256 fundersLength = s_funders.length; // Now no need to access storage on every loop that is more gas efficient.
        for (uint256 funderIndex = 0; funderIndex < fundersLength; funderIndex++) {
            address funder = s_funders[funderIndex]; // Get the address of the funder at the current index.
            s_addressToAmountFunded[funder] = 0; // Resetting the amount funded by each address to 0.
        }
    ...
    function getAddressToAmountFunded(address funder) public view returns (uint256) {
        return s_addressToAmountFunded[funder]; // To get the amount funded by a specific address.
    }
    function getFunder(uint256 index) public view returns (address) {
        return s_funders[index]; // To get the address of a specific funder.
    }
    function getFunderCount() public view returns (uint256) {
        return s_funders.length; // To get the number of funders.
    }
    function getOwner() public view returns (address) {
        return i_owner; // To get the owner of the contract.
    }
}
